...
No, it is not $\mathbf{2PL}$ Strict because $T_1$ is in its \textbf{Growing Phase} by acquiring \textbf{Shared Lock} $\mathbf{X(A)}$; however, $T_2$ holding a shared $\mathbf{S(A)}$ lock causes a conflict.
...
\begin{itemize}
    \item In this lab, the work was less challenging than previous labs. However, it was very theory heavy, so we had to learn and understand several new concepts before using them for different cases.
    
    \item During the insertion of synthetic data, we had some technical problems. The large number of insertions caused connection shut down on some machines. To solve this problem, we had to test on another machine that could handle the workload better.
\end{itemize}
...
\begin{itemize}
    This project successfully optimized the Moroccan National Health Services (MNHS) database by integrating advanced physical design and transaction management strategies.


\item \textbf{Performance Gains}: Through the strategic implementation of secondary indexes and partitioning (Range and Hash),
 query execution times were significantly reduced, achieving a  speedup on datasets of 1 million rows.


\item \textbf{Data Integrity}: The system's reliability was secured by applying ACID properties,
 implementing atomic transactions for critical operations, and resolving concurrency issues through Conflict Serializability and Deadlock analysis.\\



Ultimately, the deliverable demonstrated a tangible balance between high-speed query performance and robust data safety suitable for a large-scale healthcare environment.
\end{itemize}
