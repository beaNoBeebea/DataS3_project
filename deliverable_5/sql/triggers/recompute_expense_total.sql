...
DROP TRIGGER IF EXISTS Expense_total_insert $$
...
-- trigger for updates
DROP TRIGGER IF EXISTS Expense_total_update $$
...
ELSE
    ...
    SELECT COALESCE(SUM(S.UnitPrice),0) INTO new_total_OLD_PRESCRIPTION 
    FROM  Include_Medication I JOIN Prescription P ON P.PID = I.PID
    JOIN ClinicalActivity CA  ON CA.CAID = P.CAID
    JOIN Department D ON D.DEP_ID = CA.DEP_ID
    JOIN Stock S ON S.MID = I.MID AND S.HID = D.HID
   ...
   -- WE GET EXPENSE TOTAL FOR NEW.PID
    SELECT SUM(S.UnitPrice) INTO new_total_NEW_PRESCRIPTION
    FROM  Include_Medication I JOIN Prescription P ON P.PID = I.PID
    JOIN ClinicalActivity CA  ON CA.CAID = P.CAID
    JOIN Department D ON D.DEP_ID = CA.DEP_ID
    JOIN Stock S ON S.MID = I.MID AND S.HID = D.HID
...
--trigger for deletes
DROP TRIGGER IF EXISTS Expense_total_delete $$
...
SELECT COALESCE(SUM(S.UnitPrice),0) INTO new_total 
FROM  Include_Medication I JOIN Prescription P ON P.PID = I.PID
JOIN ClinicalActivity CA  ON CA.CAID = P.CAID
JOIN Department D ON D.DEP_ID = CA.DEP_ID
JOIN Stock S ON S.MID = I.MID AND S.HID = D.HID
